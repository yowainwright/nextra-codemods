...
export default function transformer(
  ...
  root.find(j.VariableDeclaration).forEach(path => {
    const declaration = path.node.declarations[0] as VariableDeclarator | undefined;
    if (declaration && 
        declaration.id && 
      ...
      if (declaration.init.type === 'CallExpression') {
        ...
        if (args.length > 0) {
          ...
          callExpr.arguments[0] = j.objectExpression([
            j.property('init', j.identifier('defaultShowCopyCode'), j.literal(true))
          ]) as ObjectExpression;
        }
      }
    }
    ...
    if (isEsm) {
      ...
      root.find(j.ExportDefaultDeclaration).forEach(path => {
        ...
        const importDecl: ImportDeclaration = j.importDeclaration(
          [j.importDefaultSpecifier(j.identifier('nextra')) as ImportDefaultSpecifier],
        ...
        const withNextraDecl: VariableDeclaration = j.variableDeclaration(
          ...
          [j.variableDeclarator(
            ...
            j.callExpression(
              ...
              [j.objectExpression([
                j.property('init', j.identifier('defaultShowCopyCode'), j.literal(true)) as Property
              ]) as ObjectExpression]
            ) as CallExpression
      ...
      root.find(j.AssignmentExpression, {
        ...
        const requireDecl: VariableDeclaration = j.variableDeclaration(
          ...
          [j.variableDeclarator(
            j.identifier('nextra') as Identifier,
            j.callExpression(
              j.identifier('require') as Identifier,
              [j.literal('nextra') as Literal]
            ) as CallExpression
          ) as VariableDeclarator]
        );
      ...
      if (isEsm) {
        ...
        const exportDecl: ExportDefaultDeclaration = j.exportDefaultDeclaration(
          j.callExpression(
            j.identifier('withNextra') as Identifier,
            [j.objectExpression([
              j.property('init', j.identifier('reactStrictMode'), j.literal(true)) as Property
            ]) as ObjectExpression]
          ) as CallExpression
        );
