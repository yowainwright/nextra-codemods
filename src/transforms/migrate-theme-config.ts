...
export default function transformer(
  ...
  objectExpression.forEach((path) => {
    path.node.properties.forEach((prop) => {
      // Check if the property has a key (not a spread element)
      if ("key" in prop && prop.key && "name" in prop.key) {
        // Store the property in themeConfig
        themeConfig[prop.key.name] = "value" in prop ? prop.value : null;
      }
    });
  });
  ...
  if (mdxComponentsContent) {
    fs.writeFileSync(
      path.join(projectRoot, "mdx-components.jsx"),
      mdxComponentsContent,
    );
  }
...
function generateLayoutContent(
  ...
  if (
    ...
    typeof themeConfig.docsRepositoryBase === "object" &&
    "value" in themeConfig.docsRepositoryBase
  ) {
    layoutProps.push(
      `docsRepositoryBase="${themeConfig.docsRepositoryBase.value || ""}"`,
    );
  }
...
function generateMdxComponentsContent(
  j: any,
  themeConfig: Record<string, any>,
): string | null {
