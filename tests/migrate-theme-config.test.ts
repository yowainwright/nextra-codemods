...
export default {
  ...
  const layoutFileCall = writeFileCalls.find(
    (call) => typeof call[0] === "string" && call[0].includes("layout"),
  );
...
export default {
  ...
  const writeFileCalls = (fs.writeFileSync as jest.Mock).mock.calls;
  const mdxComponentsCall = writeFileCalls.find(
    (call) => typeof call[0] === "string" && call[0].includes("mdx-components"),
  );
